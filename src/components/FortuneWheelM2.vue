...
import { db, colM12, colM22, colM32, prizeMRef2, kuponMRef2, winnerMRef2 } from '../firebase';
import PrizeList from './FortuneWheelM2/PrizeList.vue';
import VoucherKode from './FortuneWheelM2/VoucherKode.vue';
import Winner from './FortuneWheelM2/Winner.vue';
export default defineComponent({
    data() {
        return {
            username: ref(),
            openTab: 1,
            prizeList: useCollection(prizeMRef2),
            kuponList: useCollection(kuponMRef2),
            winnerList: useCollection(winnerMRef2),
            id: "",
            url: "",
            imgResult: "",
            percentage: 0,
    ...
    methods: {
            ...
            uploadString(storageRef, this.imgResult, 'data_url').then((snapshot) => {
                getDownloadURL(snapshot.ref).then((downloadURL) => {
                    ...
                    if (this.url != "" && this.percentage != null && this.prize != "" && this.color != null) {
                        setDoc(doc(db, colM12, `${parseInt(prize.id) + 1}`), data).then(() => {
                            this.id = ""
                            this.url = ""
                            this.percentage = 0
                            this.color = ""
        ...
        onUpdate() {
            ...
            updateDoc(doc(db, colM12, `${this.id}`), data).then(() => {
                this.id = ""
                this.url = ""
                this.percentage = 0
                this.color = ""
        ...
        onDelete(prize: DocumentData) {
            deleteDoc(doc(db, colM12, `${prize.id}`));
        }
    },
