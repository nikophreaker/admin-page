...
import PrizeList from './FortuneWheelM/PrizeList.vue';
import VoucherKode from './FortuneWheelM/VoucherKode.vue';
import Winner from './FortuneWheelM/Winner.vue';
export default defineComponent({
    data() {
        return {
            ...
            winnerList: useCollection(winnerMRef),
            id: "",
            url: "",
            imgResult: "",
            percentage: 0,
    ...
    methods: {
            ...
            uploadString(storageRef, this.imgResult, 'data_url').then((snapshot) => {
                getDownloadURL(snapshot.ref).then((downloadURL) => {
                    ...
                    if (this.url != "" && this.percentage != null && this.prize != "" && this.color != null) {
                        setDoc(doc(db, colM, `${parseInt(prize.id) + 1}`), data).then(() => {
                            this.id = ""
                            this.url = ""
                            this.percentage = 0
                            this.color = ""
        ...
        onDelete(prize: DocumentData) {
            deleteDoc(doc(db, colM, `${prize.id}`));
        }
    },
