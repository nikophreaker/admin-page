...
export default defineComponent({
    ...
    methods: {
        ...
        async makeCode(length: number) {
            ...
            const q = query(kuponMRef2, where("kode", "==", result));
        ...
        onAdd(kupon: DocumentData) {
            ...
            if (this.kode != "" && this.status != null) {
                setDoc(doc(db, colM22, `${id}`), data).then(() => {
                    this.id = ""
                    this.kode = ""
                    this.fixed = false
                    this.idPrize = "",
        ...
        onUpdate() {
            ...
            updateDoc(doc(db, colM22, `${this.id}`), data).then(() => {
                this.id = ""
                this.kode = ""
                this.fixed = false
                this.idPrize = ""
        ...
        onDelete() {
            deleteDoc(doc(db, colM22, `${this.id}`));
