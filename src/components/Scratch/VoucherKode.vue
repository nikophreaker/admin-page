...
import { db, colScratch2, kuponScratchRef } from '../../firebase';
export default defineComponent({
    data() {
        return {
            kuponList: useCollection(kuponScratchRef),
            id: "",
            kode: "",
            status: true,
            openAdd: false,
    ...
    methods: {
        ...
        async makeCode(length: number) {
            ...
            const q = query(kuponScratchRef, where("kode", "==", result));
            ...
            if (querySnapshot.size == 0) {
                ...
                setDoc(doc(db, colScratch2, `${id}`), data);
                return result
            } else {
                return;
            }
        ...
        onUpdate() {
            ...
            updateDoc(doc(db, colScratch2, `${this.id}`), data).then(() => {
                this.id = ""
                this.kode = ""
                this.status = true
            });
        ...
        onDelete() {
            deleteDoc(doc(db, colScratch2, `${this.id}`));
