...
import PrizeList from './FortuneWheelVip/PrizeList.vue';
import VoucherKode from './FortuneWheelVip/VoucherKode.vue';
import Winner from './FortuneWheelVip/Winner.vue';
export default defineComponent({
    ...
    methods: {
            ...
            uploadString(storageRef, this.imgResult, 'data_url').then((snapshot) => {
                getDownloadURL(snapshot.ref).then((downloadURL) => {
                    ...
                    if (this.url != "" && this.percentage != null && this.prize != "" && this.color != null) {
                        setDoc(doc(db, colVip, `${parseInt(prize.id) + 1}`), data).then(() => {
                            this.id = ""
                            this.url = ""
                            this.percentage = 0
                            this.color = ""
        ...
        onDelete(prize: DocumentData) {
            deleteDoc(doc(db, colVip, `${prize.id}`));
        }
    },
